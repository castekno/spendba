...
function determineStatus(openDateStr: string, closeDateStr: string): TenderItem['status'] {
  const now = Date.now();
...
async function fetchLiveTendersFromPTBA(): Promise<TenderItem[]> {
  ...
  const tenders: TenderItem[] = await Promise.all(
